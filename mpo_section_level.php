...
//                }
            ...
            case "B_TpDisadv":
                ...
                break;
            case "b_jobphh":
                $query = "select sum(b_hh), sum(b_jobs), sum(b_jobphh), count(OGR_FID) from b14_new where sectionnum = $i";
                $data = mysqli_query($conn, $query);
                $data = fetchAll($data);
                $t_job = $data[0]['sum(b_jobs)'];
                $t_hh = $data[0]['sum(b_hh)'];
                $coef = $data[0]['sum(b_jobphh)'];
                $div = $data[0]['count(OGR_FID)'];
                //$v = $t_job / ($t_hh*$coef);
                $toReturn['t_job'.$i] = number_format((string) floor($t_job), 0, ',', ',');
                $toReturn['t_hh'.$i] =  number_format((string) ($t_hh), 0, ',', ',');
                $toReturn['jh_ratio'.$i] = number_format($coef/$div, 2, '.', '');
                break;
            case "non-moto":
                for ($j=2012; $j <= 2016; $j++) {
                    if($i == 1){
            ...
            case "tti":
                ...
                break;
            case "crashes":
                $query_inj = "select count(OGR_FID) from c32 where incinjcras = 1 and sectnum = $i";
                $inj = mysqli_query($conn, $query_inj);
                $inj = fetchAll($inj);
                $send_inj = $inj[0]['count(OGR_FID)'];
                $toReturn['inj'.$i] = number_format($send_inj, 0, ',', ',');

                $query_fat = "select count(OGR_FID) from c32 where fatal = 1 and sectnum = $i";
                $fat = mysqli_query($conn, $query_fat);
                $fat = fetchAll($fat);
                $send_fat = $fat[0]['count(OGR_FID)'];
                $toReturn['fat'.$i] = number_format($send_fat, 0, ',', ',');
